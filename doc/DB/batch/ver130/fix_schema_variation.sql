...
BEGIN
  ...
  CASE jsonb_typeof(src)
	...
	WHEN 'string' THEN
	  ...
	  --  半角コロン（全角に変換）
	  --  全角数字（半角へ変換）
	  IF position(E'\n' in stringvalue) = 0 AND stringvalue ~ '[\(\): 　０-９]' THEN
	    RAISE INFO 'found: %', stringvalue;
	    modified := TRUE;
		stringvalue := regexp_replace(stringvalue, '[ 　]', '', 'g');
		stringvalue := translate(stringvalue, '():０１２３４５６７８９', '（）：0123456789');
		stringvalue := replace(stringvalue, '病理学的索が行われなかった', '病理学的検索が行われなかった');
